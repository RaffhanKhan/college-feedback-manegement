...
$faculity=$_REQUEST['fac'];
$vregno=$_REQUEST['vregno'];
$conn=mysqli_connect($server,$dbuname,$dbpass,$dbname); 
$sql='update report set done=1 where subject="'.$subject.'"';
...
<body  onload="noBack" onpageshow="if(event.presisted) noBack();" onunload="" style="text-align: center;background-color:#f5f2d0 ">
...
<form method="get" action="store.php">
	...
	<input type="hidden" name="subject" value="<?php print_r($subject);?>">
		<input type="hidden" name="faculity" value="<?php print_r($faculity);?>">
